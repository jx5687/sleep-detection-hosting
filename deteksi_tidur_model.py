...
model = load_model("sleep_detection_model.h5")
...
while True:
    ...
    if len(wajah) > 0:
        ...
        for (x, y, w, h) in wajah:
            ...
            for (ex, ey, ew, eh) in mata:
                ...
                prediksi = model.predict(eye_img, verbose=0)[0][0]

                # Biasanya:
                # prediksi mendekati 1 = open
                # prediksi mendekati 0 = closed
                if prediksi > 0.5:
                    label = "OPEN"
                    mata_tertutup = False
                    warna_mata = (0, 255, 0)
    ...
    cv2.imshow("AI Sleep Detection - Model Dataset", frame)
